...
import org.testies.MultiLevelIndex.Reader;


public class BypassOperations  {
  ...
  public FileSKVIterator openIndex(String file, FileSystem fs, Configuration conf, AccumuloConfiguration acuconf, BlockCache dataCache, BlockCache indexCache)
    ...
    final org.testies.RFile.Reader reader = new RFile.Reader(_cbr);
    
    return reader.getIndex();
  }
  
  
  public static FileSKVIterator openReader(String file, boolean seekToBeginning, FileSystem fs, Configuration conf, AccumuloConfiguration acuconf) throws IOException {
    return openReader(file, seekToBeginning, fs, conf, acuconf, null, null);
  }
  
  
  public static FileSKVIterator openReader(String file, boolean seekToBeginning, FileSystem fs, Configuration conf, AccumuloConfiguration acuconf,
      BlockCache dataCache, BlockCache indexCache) throws IOException {
    Path path = new Path(file);
    
    CachableBlockFile.Reader _cbr = new CachableBlockFile.Reader(fs, path, conf, dataCache, indexCache);
    org.testies.RFile.Reader iter = new RFile.Reader(_cbr);
